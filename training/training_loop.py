...
def training_loop(
    ...
    if rank == 0:
        ...
        if G_ema.synthesis.img_channels == 4:
            rgb_images = images[:, :3, :, :]
            save_image_grid(rgb_images, os.path.join(run_dir, 'rgb_fakes_init.jpg'), drange=[-1,1], grid_size=grid_size)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            if G_ema.synthesis.img_channels == 4:
                rgb_images = images[:, :3, :, :]
                save_image_grid(rgb_images, os.path.join(run_dir, f'rgb_fakes{cur_nimg//1000:06d}.jpg'), drange=[-1,1], grid_size=grid_size)
